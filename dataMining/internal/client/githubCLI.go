...
func (c *GhClient) fetchPullRequestPage(owner, name, cursor string) (*domain.PullRequestResponse, error) {
...
func (c *GhClient) RetrieveMergeCommitOid(owner, repo, base, head string) (string, error) {
	...
	var resp domain.CompareResponse
	if err := json.Unmarshal(output, &resp); err != nil {
		return "", err
	}
...
func (c *GhClient) mapPRNodes(resp *domain.PullRequestResponse, owner, name string) ([]domain.PullRequest, error) {
...
func (c *GhClient) GetRateLimit() (*domain.RateLimitResponse, error) {
	output, err := c.runGh([]string{"api", "rate_limit"})
	if err != nil {
		return nil, fmt.Errorf("get rate limit: %w", err)
	}

	var rateLimit domain.RateLimitResponse
...
func (c *GhClient) decodePullRequests(output []byte) ([]domain.PullRequest, error) {
	var prs []domain.PullRequest
